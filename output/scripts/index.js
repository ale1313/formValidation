...
var validationHandler = function validationHandler() {
  ...
  var _document$adoption = document.adoption;
  ...
  if (email.value.trim() === "") {
    ...
    errorStyling('email');
  } else if (!/(\w\.*)+@[a-z]+\.[a-z]{3}/.test(email.value) || !/[a-z]/.test(email.value.charAt(email.value.length - 1)) || email.value.match(/@/g).length > 1 || email.value.length > 320) {
  ...
  if (country.value.trim() === "") {
    ...
    for (i = 0; i <= 194; i++) {
      var optionValue = document.getElementById('countryList').children[i].value;
  ...
  if (unfilledFields.length > 0) {
    unfilledFieldsString = unfilledFields.map(function (field) {
      return unfilledFieldsString.concat("".concat(field, "\n"));
    }).join('');
    alertHandler("You must fulfill the following fields:\n".concat(unfilledFieldsString));
  }

  alertMessage === '' ? alertHandler('Form sent succesfully!', true) : alertHandler('', true);
...
var letterInputHandler = function letterInputHandler() {
  if (!/[a-z]/i.test(event.key)) {
    ...
    event.preventDefault();
    alert("The max length for the ".concat(event.target.id.toLowerCase(), " should be 17 characters"));
  }
};
...
var ageInputHandler = function ageInputHandler() {
  ...
  } else if (event.target.value.length > 2 && window.getSelection().toString() === "" || window.getSelection().toString() !== "" && window.getSelection().anchorOffset !== 10) {
    event.preventDefault();
...
var namePasteHandler = function namePasteHandler() {
  if (event.clipboardData.getData("text").length + document.adoption[event.target.name].value.length > 17) {
    event.preventDefault();
    alert("The max length for the ".concat(event.target.id.toLowerCase(), " should be 17 characters"));
  }
};
...
var writingPasteHandler = function writingPasteHandler() {
  if (event.clipboardData.getData("text").length + document.adoption[event.target.name].value.length > 300) {
    event.preventDefault();
  }
};